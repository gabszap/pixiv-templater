    ...
    if (template.ageRating) {
        setTimeout(() => {
            ...
            if (ageRatingRadio) {
                ...
                console.log("[Templater] ✓ Age rating selecionado:", template.ageRating);
                ...
                setTimeout(() => {
                    if (template.ageRating === "general") {
                        ...
                        if (adultContentRadio) {
                            ...
                            console.log("[Templater] ✓ Adult content configurado:", adultContentValue);
                        }
                        ...
                        console.log("[Templater] ✓ Mature content configurado:", template.matureContent);
                    }
                }, 300);
            }
        }, 200);
    ...
    if (template.aiGenerated) {
        setTimeout(() => {
            ...
            if (aiRadio) {
                ...
                console.log("[Templater] ✓ AI-generated configurado:", template.aiGenerated);
            }
        }, 400);
    }
...
function addTag(tagName) {
    ...
    const tagButtons = document.querySelectorAll('button[class*="gtm-"][class*="tag"]');
    ...
    if (!found) {
        console.warn("[Templater] ⚠ Tag não encontrada nos botões recomendados:", tagName);
    }
}
...
function createUI() {
        ...
        <div id="pixiv-templater">
            ...
            <div id="pixiv-templater-content">
                ...
                <div class="template-actions">
                    ...
                    <button class="action-btn" id="import-templates">📥 Importar</button>
    ...
    $header.on("mousedown", function(e) {
        ...
        if (e.target === this || $(e.target).closest("#pixiv-templater-header").length) {
            isDragging = true;
            $panel.css({
                cursor: "grabbing",
    ...
    $(document).on("mouseup", function() {
        if (isDragging) {
            ...
            // Salva a posição
            GM_setValue("templater_position", JSON.stringify({
                top: yOffset,
                left: xOffset,
            }));
        }
    ...
    function editTemplate(name, template) {
        ...
        $("#template-adult-content").prop("checked", template.adultContent || false);
        ...
        if (template.matureContent) {
            template.matureContent.forEach(content => {
                $(`#mature-content-group input[value="${content}"]`).prop("checked", true);
            });
        }
    ...
    $("#template-form").on("submit", function(e) {
        ...
        if (!editingTemplateName && templates[name]) {
            if (!confirm(`Já existe um template chamado "${name}". Deseja substituir?`)) {
                return;
            }
        }
...
function showPreview(name, template) {
    ...
    $("#preview-age-rating").text(ageRatingLabels[template.ageRating] || "All ages");
    ...
    if (template.ageRating === "general") {
        ...
        if (template.matureContent && template.matureContent.length > 0) {
            ...
            const matureLabels = template.matureContent.map(c => labels[c]).join(", ");
...
function downloadExampleTemplate() {
    const exampleTemplate = {
        "Exemplo - Genshin Impact": {
            "title": "Hu Tao ❤️",
            "caption": "Uma fanart da Hu Tao de Genshin Impact!\n\n#GenshinImpact #HuTao #Fanart",
            "tags": [
                "Genshin Impact",
                "Hu Tao",
                "原神",
        ...
        "Exemplo - Original R-18": {
            "title": "Original Character",
            "caption": "Desenho original - personagem própria\n\nAviso: conteúdo sensível",
            "tags": [
                "オリジナル",
                "original",
                "R-18",
    ...
    link.href = url;
    link.download = "pixiv-templates-exemplo.json";
    link.click();
    URL.revokeObjectURL(url);
    
    alert("📄 Template de exemplo baixado!\n\nUse este arquivo como referência para criar seus próprios templates.\n\nVocê pode editá-lo e depois importá-lo usando o botão 'Importar'.");
    console.log("[Templater] Template de exemplo gerado!");
}
